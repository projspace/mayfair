...
try 
{
	...
	$objPHPExcel->getProperties()->setTitle($config['company']." Commission Reports");
	$objPHPExcel->getProperties()->setSubject($config['company']." Commission Reports");
	...
	$objPHPExcel->getActiveSheet()->setCellValue('E2', 'Order ID');
	$objPHPExcel->getActiveSheet()->setCellValue('F2', 'Date');
	$objPHPExcel->getActiveSheet()->setCellValue('G2', 'Total - excluding shipping and packing($)');
	...
	while($row = $reports->FetchRow())
	{
		...
		foreach($report_orders as $order)
			...
			{
				$objPHPExcel->getActiveSheet()->setCellValue('E'.$row_index, $order['id']);
				$objPHPExcel->getActiveSheet()->setCellValue('F'.$row_index, date('d/m/Y H:i', $order['time']));
				$objPHPExcel->getActiveSheet()->setCellValue('G'.$row_index, round($order['total'], 2));
				$objPHPExcel->getActiveSheet()->setCellValue('H'.$row_index, round($order['commission'], 2));
				$row_index++;
			}
	}
	
	$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
	$objWriter->save($config['path'].'downloads/reports/commission-reports-'.safe($_REQUEST['src']).'-'.date('d.m.Y-H.i.s').'.xlsx');
	
	header('Location: '.$config['dir'].'downloads/reports/commission-reports-'.safe($_REQUEST['src']).'-'.date('d.m.Y-H.i.s').'.xlsx');
	exit;
}
catch (Exception $e)
{
