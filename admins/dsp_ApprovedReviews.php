	...
	while($row=$reviews->FetchRow())
	{
		...
		if($acl->check("removeProductReview"))
			echo "<a class=\"button button-grey\" title=\"Remove\" onclick=\"return postbackConf(
				...
				,[{$row['id']},{$row['product_id']},{$row['category_id']},'approved']
				,'remove'
				,'review')\"><span>Remove</span></a>\n";
		if($acl->check("editProductReview"))
			echo "<a class=\"button button-grey\" href=\"{$config['dir']}index.php?fuseaction=admin.editProductReview&amp;review_id={$row['id']}&amp;product_id={$row['product_id']}&amp;category_id={$row['category_id']}&amp;return=approved\"><span>Edit</span></a>\n";
		echo "</td>
		</tr>";
	}
?>
...
<?
	...
	if($nr_pages > 1)
	{
		...
		echo '<div style="width:100%;" class="paginator"><ul>';
			...
			$results_page = array();
			$results_page[] = 'fuseaction=admin.approvedReviews';
