...
<form method="post" enctype="multipart/form-data" action="<?= $config['dir'] ?>index.php?fuseaction=admin.editColor&amp;color_id=<?=$color['id'] ?>&amp;act=update">
	<div id="tabs" class="form clearfix">
		...
		<div id="tabs-1">
			...
			<div class="form-field clearfix">
				...
				<input type="text" id="code" name="code" value="<?=$color['code'] ?>" />
			</div>
			<div class="form-field clearfix">
				<label for="hexa">Color<em>(hex code, ie f3g4g5)</em></label>
				<input type="text" id="hexa" name="hexa" value="<?=$color['hexa'] ?>" />
			</div>
			<div class="form-field clearfix">
				...
				<?
					if($color['image_type']!="")
						echo "<label>&nbsp;</label><img src=\"{$config['dir']}images/colors/{$color['id']}.{$color['image_type']}?time=".time()."\" width=\"20\" /><br />";
				?>
			</div>
		</div>
	</div>
