...
html = html.replace(/(<a [^>]*href=")#([^"]*)"([^>]*>)/gi, "$1javascript:document.location.replace(String(document.location)+'#$2')\"$3");
return html;
}
function wproPlugin_wproCore_bookmark_df(editor, html) {
html = html.replace(/(<a [^>]*href=")javascript:document\.location\.replace\(String\(document\.location\)\+'#([^']*)'\)"([^>]*>)/gi, "$1#$2\"$3");
return html;
}
