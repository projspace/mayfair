...
/* <![CDATA[ */
	...
	function add_price(){
		...
		$('#table_prices').append('<tr class="'+row_class+'" id="'+next_id+'"><td><input type="hidden" name="weight[]" value="'+weight+'" class="hidden_weight" />'+weight+'g</td><td><input type="hidden" name="price[]" value="'+price+'" class="hidden_price" />$'+price+'</td><td class="right"><a href="#" class=" button button-grey remove_row"><span>Remove</span></a></td></tr>');
		$('.remove_row').click(function(){
			$(this).parent().parent().remove();
			return false;
		});
...
<h1>Add Shipping Area</h1>
<form method="post" action="<?= $config['dir'] ?>index.php?fuseaction=admin.addArea&act=add">
	<div id="tabs" class="form clearfix">
		...
		<div id="tabs-1">
			...
			<div class="form-field clearfix">
				...
				<input type="text" id="over_weight_unit" name="over_weight_unit" />
			</div>
			<div class="form-field clearfix">
				<label for="over_price">Price per additional weight unit ($)</label>				
				<input type="text" id="over_price" name="over_price" />
			</div>
			<div class="form-field clearfix">
				<label for="free_shipping">Free shipping minimum order amount ($)</label>				
				<input type="text" id="free_shipping" name="free_shipping" />
			</div>
		</div>
		<div id="tabs-2">
