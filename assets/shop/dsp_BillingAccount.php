...
<?
	$content_area = $elems->qry_ContentArea(6);
	echo $content_area['description'];
?>
</article>
<article id="shopContent">
	<div class="splitTwo clearfix">
		<h3>Choose an existing billing address</h3>
	<?
		$addresses = array();
		foreach($billing as $index=>$row)
			$addresses[$index] = '
				<form action="'.$config['dir'].'billing?act=chooseAddress" method="post">
					...
					<input type="submit" value="Choose this address & Pay" class="grayButton" />
				</form>
			';
		$billing = array();
		for($i=0;$i<count($addresses);$i+=2)
		{
			if(isset($addresses[$i+1]))
				$billing[] = '<div class="deliveryAddress secondRow">'.$addresses[$i+1].'</div>';
			$billing[] = '<div class="deliveryAddress">'.$addresses[$i].'</div>';
		}
		echo implode('', $billing);
	?>
	</div>

	<hr />

	<form id="quickCheckout" action="<?=$config['dir'] ?>billing?act=saveAddress" method="post" class="splitTwo">
		<h3>or add a new one</h3>
		<p class="row">
			<span><input type="text" value="Full Name" placeholder="Full Name" id="name" name="name" tabindex="1" /></span>
			<span><input type="text" value="Email" placeholder="Email" id="email" name="email" tabindex="8" /></span>
			<br clear="all"/>
		</p>
		<p class="row">
			<span><input type="text" value="Address Line 1" placeholder="Address Line 1" id="line1" name="line1" tabindex="2" /></span>
			<span><input type="text" value="Telephone" placeholder="Telephone" id="phone" name="phone" tabindex="9" /></span>
			<br clear="all"/>
		</p>
		<p class="row">
		...
		<br clear="all"/>
		<p class="row">
			<input type="submit" value="Use this address & Pay" class="grayButton" tabindex="8" />
		</p>
	</form>
	<br/>
	<br/>
