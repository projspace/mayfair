...
/* <![CDATA[ */
	$(document).ready(function(){
		...
		$('#cboxWrapper iframe', top.document).css('overflow', 'visible');
		$('html', document).css('overflow-y', 'visible');
	});
/* ]]> */
</script>
<form id="advancedSearch" action="<?=$config['dir'] ?>search" method="get" class="advancedForm" target="_parent">
	...
	<p class="row"><input type="text" id="keyword" name="keyword" value="<?=(safe($_GET['keyword']) != '')?safe($_GET['keyword']):'Text in name / description of product' ?>" placeholder="Text in name / description of product" /></p>
	<p class="row"><input type="text" id="tag" name="tag" value="<?=(safe($_GET['tag']) != '')?safe($_GET['tag']):'Type of Product' ?>" placeholder="Type of Product" /></p>
	...
	<div style="width: 500px;">
		<ul class="selector float">
			<li style="margin: 0 0 5px 0; width: 240px;"><a class="arrowed" href="#" style="width: 234px;">Related to this Play</a><dl style="display: none;">
			<?
				$type = 'play';
				$values = $filters[$type];
			...
			?>
			</dl><input type="hidden" name="filter_ids[<?=$type ?>]" value="" /></li>
			<li style="margin: 0 0 5px 0; width: 240px; float: right;"><a class="arrowed" href="#" style="width: 234px;">Related to this Character</a><dl style="display: none;">
			<?
				$type = 'character';
				$values = $filters[$type];
				if(is_array($values) && count($values))
			...
			<li style="margin: 0 0 5px 0; width: 240px;"><a class="arrowed" href="#" style="width: 234px;">Related to this Director</a><dl style="display: none;">
			<?
				$type = 'director';
				$values = $filters[$type];
				if(is_array($values) && count($values))
			...
			<li style="margin: 0 0 5px 0; width: 240px; float: right;"><a class="arrowed" href="#" style="width: 234px;">Related to this Actor</a><dl style="display: none;">
			<?
				$type = 'actor';
				$values = $filters[$type];
				if(is_array($values) && count($values))
