...
<h1 class="pageTitle">
	<? if($acl->check("approvedReviews")): ?><a class="button button-grey" title="Reviews (approved)" href="<?=$config['dir'] ?>index.php?fuseaction=admin.approvedReviews" rel="import-container"><span>Reviews (approved)</span></a><? endif; ?>
	<? if($acl->check("rejectedReviews")): ?><a class="button button-grey" title="Reviews (cancelled)" href="<?=$config['dir'] ?>index.php?fuseaction=admin.rejectedReviews" rel="import-container"><span>Reviews (cancelled)</span></a><? endif; ?>
	Pending Reviews
</h1>
	...
	while($row=$reviews->FetchRow())
	{
		...
		if($acl->check("removeProductReview"))
			echo "<a class=\"button button-grey\" title=\"Remove\" onclick=\"return postbackConf(
				...
				,[{$row['id']},{$row['product_id']},{$row['category_id']},'reviews']
				,'remove'
				,'review')\"><span>Remove</span></a>\n";
		if($acl->check("editProductReview"))
			echo "<a class=\"button button-grey\" href=\"{$config['dir']}index.php?fuseaction=admin.editProductReview&amp;review_id={$row['id']}&amp;product_id={$row['product_id']}&amp;category_id={$row['category_id']}&amp;return=reviews\"><span>Edit</span></a>\n";
		echo "</td>
		</tr>";
	}
?>
...
<?
	...
	if($nr_pages > 1)
	{
		...
		echo '<div style="width:100%;" class="paginator"><ul>';
			...
			$results_page = array();
			$results_page[] = 'fuseaction=admin.reviews';
