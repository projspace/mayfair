	...
	$reports = $db->Execute(
		$sql = sprintf("
			SELECT
				...
				,SUM(IF(shop_user_accounts.id = shop_promotional_codes.shop_account_id, shop_promotional_codes.shop_commission, shop_promotional_codes.teacher_commission) * (shop_orders.total - shop_orders.promotional_discount) / 100) total
			FROM
				shop_user_accounts
			...
			AND
				shop_promotional_codes.id = shop_orders.discount_code_id
			AND
				%s
			GROUP BY
				shop_user_accounts.id
	...
	$report_orders = $db->Execute(
		$sql = sprintf("
			SELECT
				...
				,IF(shop_user_accounts.id = shop_promotional_codes.shop_account_id, shop_promotional_codes.shop_commission, shop_promotional_codes.teacher_commission) * (shop_orders.total - shop_orders.promotional_discount) / 100 commission
			FROM
				shop_user_accounts
				,shop_promotional_codes
				,shop_orders
