<?
	...
	$cart=$db->Execute(
		$sql = sprintf("
			SELECT DISTINCT
				...
				,shop_products.vat
				,shop_products.flat_rate_shipping
	...
	/*if($shippable)
	{
		...
		if($delivery_service)
		{
			...
			if($flat_rate_shipping && $vars['total'] < 50)
			{
				$vars['shipping'] += $flat_rate_shipping_value;
				if($services)
					foreach($services as $code=>$row)
						$services[$code]['price'] += $flat_rate_shipping_value;
			}
		}
		else
		{
